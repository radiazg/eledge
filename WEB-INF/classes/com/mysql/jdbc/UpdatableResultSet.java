...
public class UpdatableResultSet extends ResultSet {
    ...
    public synchronized void deleteRow() throws SQLException {
        ...
        if (this.onInsertRow) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.1")); //$NON-NLS-1$
        } else if (this.rowData.size() == 0) {
            throw new SQLException(Messages.getString("UpdatableResultSet.2")); //$NON-NLS-1$
        } else if (isBeforeFirst()) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.3")); //$NON-NLS-1$
        } else if (isAfterLast()) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.4")); //$NON-NLS-1$
        }
        ...
        if (this.deleter == null) {
            ...
            this.deleter = (com.mysql.jdbc.PreparedStatement) this.connection
        ...
        try {
			...
			throw new SQLException(Messages.getString("UpdatableResultSet.39", new Object[] {this.charEncoding}) //$NON-NLS-1$
    ...
    public synchronized void moveToInsertRow() throws SQLException {
        ...
        if (this.inserter == null) {
			...
			this.inserter = (com.mysql.jdbc.PreparedStatement) this.connection
    ...
    public synchronized void refreshRow() throws SQLException {
        ...
        if (this.onInsertRow) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.8")); //$NON-NLS-1$
        } else if (this.rowData.size() == 0) {
            throw new SQLException(Messages.getString("UpdatableResultSet.9")); //$NON-NLS-1$
        } else if (isBeforeFirst()) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.10")); //$NON-NLS-1$
        } else if (isAfterLast()) {
            throw new SQLException(
                Messages.getString("UpdatableResultSet.11")); //$NON-NLS-1$
        }
        ...
        if (this.refresher == null) {
			...
			this.refresher = (com.mysql.jdbc.PreparedStatement) this.connection
        ...
        try {
            ...
            if (rs.next()) {
                ...
                throw new SQLException(Messages.getString("UpdatableResultSet.12"), //$NON-NLS-1$
    ...
    protected synchronized void generateStatements() throws SQLException {
        ...
        if (this.fields[0].getOriginalTableName() != null) {
            ...
            tableNameBuffer.append(quotedId);
            tableNameBuffer.append(this.fields[0].getOriginalTableName());
            tableNameBuffer.append(quotedId);
            ...
            tableNameBuffer.append(quotedId);
            tableNameBuffer.append(this.fields[0].getTableName());
            tableNameBuffer.append(quotedId);
		...
		this.updateSQL = "UPDATE " + this.tableName + " " + fieldValues.toString() //$NON-NLS-1$ //$NON-NLS-2$
            + " WHERE " + keyValues.toString(); //$NON-NLS-1$
		this.insertSQL = "INSERT INTO " + this.tableName + " (" + columnNames.toString() //$NON-NLS-1$ //$NON-NLS-2$
            + ") VALUES (" + insertPlaceHolders.toString() + ")"; //$NON-NLS-1$ //$NON-NLS-2$
		this.refreshSQL = "SELECT " + columnNames.toString() + " FROM " + this.tableName //$NON-NLS-1$ //$NON-NLS-2$
            + " WHERE " + keyValues.toString(); //$NON-NLS-1$
		this.deleteSQL = "DELETE FROM " + this.tableName + " WHERE " //$NON-NLS-1$ //$NON-NLS-2$
            + keyValues.toString();
    }
		...
		if (this.connection.getUseUsageAdvisor()) {
			if (this.deleter == null && this.inserter == null && this.refresher == null && this.updater == null) {
				...
				this.eventSink.consumeEvent(new ProfilerEvent(
										...
										this.resultId, System.currentTimeMillis(), 0,
    ...
    synchronized void syncUpdate() throws SQLException {
        if (this.updater == null) {
			...
			this.updater = (com.mysql.jdbc.PreparedStatement) this.connection
        ...
        if (numKeys == 1) {
            ...
            for (int i = 0; i < numKeys; i++) {
                ...
                if (currentVal != null) {
					this.updater.setBytes(numFields + i + 1, currentVal);
                } else {
    ...
    private void checkUpdatability() throws SQLException {
        ...
        if ((this.catalog == null) || (this.catalog.length() == 0)) {
            ...
            if ((this.catalog == null) || (this.catalog.length() == 0)) {
                throw new SQLException(
                    Messages.getString("UpdatableResultSet.43") //$NON-NLS-1$
                    , SQLError.SQL_STATE_ILLEGAL_ARGUMENT); //$NON-NLS-1$ //$NON-NLS-2$
            }
        }
