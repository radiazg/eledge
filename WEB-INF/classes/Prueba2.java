...
public class Prueba2 extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request,HttpServletResponse response)
    ...
    if (id_course != null) {
    	if(student.isRegisteredCourse(student.getIDNumber(),Integer.parseInt(id_course)) || (student.getIsInstructor()))
    		student.setCourse_id(id_course);
    	}		
    }
       
    /*String Frame = "<head>"
    +	"<title>PT04: material de prueba</title>"
    +"</head>"
    +"<frameset rows='130,*' border='2'>"
    +	"<frame src='http://localhost'>"
    +	"<frame src='http://localhost:8080/Eledge/Content/1.htm'>"
    +"</frameset>"; */
    
    //out.print(Frame);   
    //out.println(Page.create(homePage(student.getIsInstructor(),(student.getCourse_id() != null?Integer.parseInt(student.getCourse_id()):0)),student));//imprime en la web el contenido de homepage     
	
	File prueba = new File("c:\\");
	File [] lista; 
	
	//prueba.mkdir();//crea el directorio
	
	/*File txt = new File("C:\\hola\\hola1.xml");
	
	FileWriter salida = new FileWriter(txt);*/
	lista = prueba.listFiles();
	
	
	for (int i=0; i< lista.length; i++){
		if(lista[i].isDirectory())
			out.println("<strong>"+lista[i].getName()+"</strong><br>\n");
		else
			out.println(lista[i].getName()+"<br>\n");
	}
	
	/*salida.write(todoxml);
	salida.close();*/
  ...
  String homePage(boolean isInstructor, int section){
    ...
    String webPage = new Page().webPage("Home",isInstructor,section);
    ...
    if (webPage == null) {
      try {
        ...
        webPage = createDefaultHomePage(stmt);
      }
      catch (Exception e) {
  ...
  String createDefaultHomePage(Statement stmt) {
